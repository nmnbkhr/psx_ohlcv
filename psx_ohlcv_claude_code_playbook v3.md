...
```
Create src/pakfindata/settings.py using pydantic-settings or plain dataclass:
...
VERIFY:
  python -c "from pakfindata.settings import Settings; s = Settings(); print(f'DB: {s.db_path}')"
  pytest tests/ -x --tb=short -q 2>&1 | tail -5
```
...
IF REDIS_OK — Use ARQ (async Redis queue):
  ...
  Create src/pakfindata/worker_arq.py with:
  ...
  VERIFY:
    arq pakfindata.worker_arq.WorkerSettings &
    sleep 2
    python -c "
    import asyncio
...
Create src/pakfindata/settings.py using pydantic-settings:
...
VERIFY:
  python -c "from pakfindata.settings import Settings; s = Settings(); print(f'DB: {s.db_backend}, Data: {s.data_root}')"
```
