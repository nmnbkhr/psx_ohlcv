...
Project structure: `src/psx_ohlcv/` with sub-packages:
...
## Step 1 — Database: `src/psx_ohlcv/db/repositories/global_rates.py`
...
```python
# In src/psx_ohlcv/db/repositories/global_rates.py
...
```python
# In src/psx_ohlcv/db/repositories/global_rates.py — add to ensure_tables()
...
class GlobalRatesScraper:
    ...
    def scrape_eustr_stub(self) -> list[dict]:
        """STUB: ECB €STR — implement when needed.
        ...
        """
        logger.warning("€STR scraper not yet implemented — stub only")
        return []
    ...
    def sync_all(self, con) -> dict:
        ...
        from psx_ohlcv.db.repositories.global_rates import ensure_tables, upsert_global_rate
...
def register_globalrates_subparser(subparsers):
    ...
    gr_parser = subparsers.add_parser('globalrates', help='Global reference rates (SOFR, SONIA, €STR, TONA)')
...
def cmd_globalrates_latest(args):
    from psx_ohlcv.db.connection import connect
    from psx_ohlcv.db.repositories.global_rates import get_all_latest_rates
...
from psx_ohlcv.db.connection import connect
from psx_ohlcv.db.repositories import global_rates as gr_repo
...
## Step 6 — Streamlit Page: `src/psx_ohlcv/ui/page_views/global_rates.py`
...
- Show latest SOFR, EFFR, KIBOR, policy rate, SONIA (if available), €STR (if available) in a summary table
