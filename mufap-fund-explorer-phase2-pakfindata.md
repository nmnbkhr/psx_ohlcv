...
| 9 | `sector` column missing from `mutual_funds` | ALTER TABLE to add it |
...
Add ONLY the 2 missing columns (7 others already exist):
...
**Already exist (do NOT add again):** expense_ratio, front_load, back_load, rating, benchmark, risk_profile, fund_id
...
3. **ALTER TABLE safely** — only `aum` and `sector` are missing. Check `PRAGMA table_info()` before adding. SQLite errors on duplicate ADD COLUMN.
...
10. **All imports use `pakfindata`** — the package was renamed from psx_ohlcv. Use `from pakfindata.xxx import yyy` everywhere.
...
git commit -m "feat: MUFAP fund explorer — full 36-category coverage + performance persistence
  ...
  - mutual_funds extended: aum, sector
