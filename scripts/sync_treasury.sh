...
# Cron: 0 16 * * 5 ~/pakfindata/scripts/sync_treasury.sh (weekly Friday)
...
$CONDA_BIN run -n "$CONDA_ENV" \
    python -m pakfindata.cli --db "$DB_PATH" treasury tbill-sync >> "$LOG_FILE" 2>&1 || true

$CONDA_BIN run -n "$CONDA_ENV" \
    python -m pakfindata.cli --db "$DB_PATH" treasury pib-sync >> "$LOG_FILE" 2>&1 || true

$CONDA_BIN run -n "$CONDA_ENV" \
    python -m pakfindata.cli --db "$DB_PATH" treasury gis-sync >> "$LOG_FILE" 2>&1 || true
