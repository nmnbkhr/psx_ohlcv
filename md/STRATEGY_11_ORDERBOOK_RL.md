# Claude Code Prompt: Strategy 11 — Deep Order Book Simulation & RL Limit Order Agent

## Context

pakfindata has raw WebSocket JSONL logs with top-of-book data (best bid, best ask, 
bidVol, askVol, price, volume, trades) for 500+ PSX symbols. The full order book 
depth is NOT available from PSX WebSocket — only Level 1 (top of book).

This strategy works around that constraint by:
1. Reconstructing approximate order book depth from top-of-book snapshots over time
2. Building an agent-based simulation of PSX market microstructure
3. Training an RL agent (PPO/DQN) to optimize limit order placement
4. Using PyTorch on RTX 4080 for training

**The research question:** Can we infer order book shape from Level 1 data + trade flow, 
then use that to place limit orders that get filled at better prices than market orders?

**What we have (Level 1):**
- Best bid price + volume (bidVol)
- Best ask price + volume (askVol)  
- Last trade price + volume
- Trade count per tick
- Timestamps at ~1-second granularity
- 4.6M+ ticks across months

**What we DON'T have (Level 2+):**
- Order book depth beyond best bid/ask
- Individual order IDs
- Order modifications/cancellations
- Hidden/iceberg orders

**Why it's still valuable on PSX:**
- PSX has ~5-10 price levels of meaningful depth (thin book)
- Top-of-book captures 60-80% of the information on PSX
- Spread is typically 1-3 ticks — the book is shallow
- No HFT means the reconstructed book is more stable/predictable

## What already exists

```bash
# Check raw WebSocket JSONL files
ls /mnt/e/psxdata/tick_logs_cloud/ | tail -5
wc -l /mnt/e/psxdata/tick_logs_cloud/*.jsonl 2>/dev/null | tail -3

# Sample raw tick data structure
python3 -c "
import json
files = sorted(__import__('pathlib').Path('/mnt/e/psxdata/tick_logs_cloud').glob('*.jsonl'))
if files:
...
# Check DuckDB tick_logs
python3 -c "
import duckdb
con = duckdb.connect('/mnt/e/psxdata/pakfindata.duckdb', read_only=True)
print(con.execute('DESCRIBE tick_logs').df().to_string())
print()
print(con.execute('SELECT COUNT(*), MIN(date), MAX(date) FROM tick_logs').fetchone())
print()
...
**READ ALL OUTPUT — confirm tick data structure and GPU availability.**

## Step 1: Install dependencies

```bash
conda activate psx
pip install torch gymnasium stable-baselines3 --break-system-packages 2>/dev/null || \
pip install torch gymnasium stable-baselines3
```

## Step 2: Create the Order Book Reconstruction Engine

Create `src/pakfindata/engine/orderbook_sim.py`:
...
class OrderBookReconstructor:
    ...
    def update(self, tick: dict) -> ReconstructedBook:
        ...
        if bid > 0 and bid_vol > 0:
            ...
            # If bid moved DOWN, the old best was consumed
            # If bid moved UP, new aggressive buyer arrived
    ...
    def _infer_levels(self, best_price: float, best_vol: int, 
        ...
        Volume at level k ≈ best_vol × α^k where α ∈ (0.3, 0.7)
...
def train_rl_agent(
    ...
    print(f"Training {algorithm} on {symbol} for {total_timesteps} timesteps...")
...
Create `src/pakfindata/ui/page_views/strategy_orderbook.py`:

### Tab 1: Book Visualization
```
├── Real-time reconstructed order book display:
│   ├── Left: Bid side (green bars, price × volume)
│   ├── Right: Ask side (red bars, price × volume)
│   ├── Center: mid price, spread, imbalance gauge
│   └── Updates from historical tick replay
├── Book heatmap: price (Y) × time (X), color = volume at each level
├── Imbalance over time (line chart, -1 to +1)
├── Spread over time (line chart, in ticks)
├── Symbol selector + date picker
└── Data quality metrics: avg depth, imbalance accuracy
```

### Tab 2: Market Simulation
```
├── Agent configuration:
│   ├── Number of noise traders (1-20)
│   ├── Number of momentum traders (1-5)
│   ├── Market maker (on/off)
│   └── [Run Simulation] button
├── Simulation results:
│   ├── Price path (simulated vs actual)
│   ├── Volume profile comparison
│   ├── Agent trade log
│   ├── Market maker P&L
│   └── Spread dynamics under different agent mixes
```
...
│   ├── Algorithm: PPO / DQN
│   ├── Symbol selector
│   ├── Training timesteps: 50K / 100K / 500K
│   ├── [Train Agent] button + progress bar
│   └── Training curve (reward over episodes)
├── Evaluation:
│   ├── Action distribution (pie chart: hold/market/limit)
│   ├── Limit vs Market fill comparison
│   ├── Average execution price vs VWAP
│   ├── Position over time
│   ├── Cumulative PnL
│   └── "How much better than random?" comparison
```

### Tab 4: Research
```
├── Level 1 sufficiency analysis:
│   ├── Imbalance → next-tick direction accuracy
│   ├── Spread × imbalance interaction
│   ├── Volume at best vs total volume correlation
│   ├── How PSX compares to developed markets (thinner book = more L1 info)
│   └── Time-of-day patterns in book quality
├── Order book shape inference:
│   ├── Power law fit: does volume decay exponentially with distance?
│   ├── Reconstructed vs actual spread comparison
│   └── Calibration: how to set decay parameters
├── Limitations & future work:
│   ├── Level 2 data sources (if PSX ever provides)
...
```python
st.page_link("page_views/strategy_orderbook.py", label="Order Book Sim", icon="📖")
```
...
# Test book reconstruction
python3 -c "
import sys; sys.path.insert(0, 'src')
from pakfindata.engine.orderbook_sim import reconstruct_book_history, analyze_book_quality

books = reconstruct_book_history('OGDC')
print(f'Book snapshots: {len(books)}')
if books:
    b = books[-1]
    print(f'Mid: {b.mid_price:.2f}, Spread: {b.spread:.2f}, Imbalance: {b.imbalance:+.3f}')
    print(f'Bids: {len(b.bids)} levels, Asks: {len(b.asks)} levels')
    for i, lvl in enumerate(b.bids[:5]):
        print(f'  Bid {i}: {lvl.price:.2f} × {lvl.volume:,}')
    for i, lvl in enumerate(b.asks[:5]):
        print(f'  Ask {i}: {lvl.price:.2f} × {lvl.volume:,}')

# Quality analysis
quality = analyze_book_quality('OGDC')
print(f'\nBook Quality:')
...
python3 -c "
import sys; sys.path.insert(0, 'src')
from pakfindata.engine.orderbook_sim import PSXMarketSimulator, NoiseTrader, MomentumTrader, MarketMaker

sim = PSXMarketSimulator('OGDC')
sim.add_agent(NoiseTrader('noise_1', trade_prob=0.05))
sim.add_agent(NoiseTrader('noise_2', trade_prob=0.03))
sim.add_agent(MomentumTrader('momentum_1'))
sim.add_agent(MarketMaker('mm_1'))

result = sim.run(max_ticks=5000)
print(f'Simulation: {result}')
"

# Test RL environment (no training)
python3 -c "
import sys; sys.path.insert(0, 'src')
from pakfindata.engine.orderbook_sim import create_rl_environment
import numpy as np

env = create_rl_environment('OGDC')
if env:
    obs, _ = env.reset()
    ...
    print(f'Random agent: {steps} steps, reward {total_reward:.4f}, trades {info.get(\"trades\", 0)}')
"

# Test RL training (short)
python3 -c "
import sys; sys.path.insert(0, 'src')
from pakfindata.engine.orderbook_sim import train_rl_agent

result = train_rl_agent('OGDC', total_timesteps=5000, algorithm='PPO')
...
1. **Level 1 only** — we DON'T have full order book depth. Reconstruction is APPROXIMATE.
2. **Power law assumption** — volume at level k = best_vol × 0.5^k. Calibrate with real data.
3. **Decay rate 0.95** — non-best levels lose 5% volume per tick (cancellation proxy).
4. **PSX tick size Rs 0.01** — most liquid stocks have 1-5 tick spreads.
5. **RL state space 14 dimensions** — intentionally compact for fast training.
6. **7 discrete actions** — hold, market buy/sell, limit buy/sell at best, limit buy/sell passive.
7. **Reward design:** PnL + spread capture bonus - impact cost penalty - inventory penalty.
8. **Train on GPU** — `device="cuda"` uses RTX 4080. Training 100K steps takes ~2-5 minutes.
9. **This is RESEARCH grade** — not production-ready. The reconstructed book is an approximation.
10. **Imbalance predictiveness** is the key metric — if L1 imbalance predicts direction >55%, the reconstruction has value.
11. **Agent-based sim** has 3 agent types: noise (retail), momentum (herding), market maker.
12. **Stable-Baselines3** for RL — PPO for continuous, DQN for discrete. Both work here.
13. **No TA libraries** — PyTorch + gymnasium + stable-baselines3 + numpy.
14. **Add under STRATEGIES** (or RESEARCH) in sidebar.
15. **Future upgrade:** If PSX ever provides Level 2 data, swap the reconstruction with real depth.
