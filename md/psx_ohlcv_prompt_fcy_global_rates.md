...
Register in `db/__init__.py`:
```python
from .global_rates import (
...
@cli.group()
def globalrates():
    """Global reference rates (SOFR, SONIA, €STR, TONA)."""
    pass

@globalrates.command('sync')
...
def sync_global_rates(count):
    ...
    con.close()
    click.echo(f"Sync complete: {stats}")
...
def sofr_kibor_spread(days):
    ...
    con.close()
    if df.empty:
        click.echo("No spread data. Ensure both KIBOR and SOFR are synced.")
    else:
        click.echo(df.to_string(index=False))

@globalrates.command('history')
@click.argument('rate_name', default='SOFR')
...
def rate_history(rate_name, days):
    ...
    con = get_db()
    start = (datetime.now() - timedelta(days=days)).strftime('%Y-%m-%d')
    df = get_rate_history(con, rate_name=rate_name.upper(), start_date=start)
    con.close()
    if df.empty:
        click.echo(f"No data for {rate_name}")
    else:
        click.echo(df.to_string(index=False))
```
