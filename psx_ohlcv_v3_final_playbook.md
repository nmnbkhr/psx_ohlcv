...
## 1.3 Starting Point (v2.1.0 — COMPLETED)

| What | Status |
|------|--------|
| Tests | 707 passing, 1 skipped |
| DB | 303.9 MB, 917K EOD rows, 189 indexes |
| Async sync | Battle-tested on 540+ symbols, cron at 18:30 PKT Mon-Fri |
| PSXClient | Dual-mode API/direct-DB, all UI pages migrated |
| Live Market page | Auto-refresh, KSE-100, sector heatmap |
| Data Quality page | Coverage, freshness, gaps, duplicates, maintenance buttons |
| Cron | Daily async sync + Friday maintenance |
| Tag | v2.1.0 pushed |

## 1.4 Success Criteria for v3.0
...
| 3 | Unified sync completes <10 min for all domains | Timed via cron logs |
...
| 6 | 17 existing Streamlit pages + 4 new pages work | UI starts without errors |
| 7 | 750+ tests pass (707 existing + new) | `pytest tests/ -q` all green |
| 8 | Extended cron covers all data domains | `crontab -l` shows 8+ jobs |
| 9 | PSXClient extended for new data (ETF, treasury, FX) | client.get_etf_data() works |
| 10 | Data Quality page shows new domains | All 34 domains visible |
...
### Prompt 3.4 — Extend Data Quality Page for New Domains
```
TASK: Extend the EXISTING Data Quality page (built in v2.1) to show new data domains.

The Data Quality page already exists from v2.1 with coverage, freshness, 
gaps, duplicates, and maintenance buttons.

Update it to include rows for:
  - ETF NAV (etf_nav table)
  - T-Bill Auctions (tbill_auctions)
  - PIB Auctions (pib_auctions)
  - PKRV Yield Curve (pkrv_daily)
  - KONIA Rates (konia_daily)
  - SBP FX Interbank (sbp_fx_interbank)
  - SBP FX Open Market (sbp_fx_open_market)
  - Kerb FX (forex_kerb)
  - IPO Calendar (ipo_listings)
  - Dividend History (dividend_history or company_payouts)

Also extend PSXClient with methods for new data:
  client.get_etf_list()
  client.get_etf_nav(symbol)
  client.get_tbill_auctions()
  client.get_fx_rates(currency, source)
  client.get_yield_curve(type)
  client.get_data_freshness()  # extended for new domains

Commit: "feat: extend Data Quality page + PSXClient for all new data domains"
```
...
## Starting Point: v2.1.0 DONE
- 707 tests, async sync working, PSXClient built, cron running
- Live Market + Data Quality pages shipped
- All UI pages wired through PSXClient

## v3.0 Execution (from v2.1.0 baseline)
...
PHASE 3: UI + API (2 days, prompts 3.1–3.5)
  ...
  Day 9: Fund explorer (3.3) + Extend Data Quality (3.4) + Research terminal (3.5)

PHASE 4: Automation + Deploy (1 day, prompts 4.1–4.4)
  Day 10: Extend cron (4.1) + Tests (4.2) + API routes (4.3) + Tag v3.0.0 (4.4)

TOTAL: 10 days × 3-4 hours per day = 30-40 Claude Code hours
```

## Key v2.1 → v3.0 Integration Points

| v2.1 Component | How v3.0 Extends It |
|----------------|---------------------|
| PSXClient | Add methods: get_etf_data(), get_tbill_auctions(), get_fx_rates(), get_yield_curve() |
| Data Quality page | Add rows for 10 new data domains (ETF, treasury, FX, etc.) |
| Cron (daily_sync.sh) | Extend with new sync scripts for treasury, FX, ETF, funds |
| Async sync pipeline | New scrapers plug into same asyncio.Queue pattern |
| FastAPI | Add 4 new routers: treasury, funds, rates, market |
| Test suite (707) | Target 750+ with new module tests |

---

# APPENDIX A: DATA SOURCE REFERENCE

## A.1 Current Data Audit (17 Existing Domains)

| # | Asset Class | Tables | Scraper | Data | Source |
|---|-------------|:---:|:---:|:---:|---|
...
| 11 | **Bonds (Corporate)** | ✅ bonds_master, bond_quotes | ✅ | ✅ | dps.psx.com.pk/debt/{SYM} |
| 12 | **Sukuk** | ✅ sukuk_master, sukuk_quotes | ✅ | ✅ | dps.psx.com.pk + SBP |
...
## A.2 New Domains to Build (16 Missing)

| # | Data Domain | Source | Priority | Prompt |
|---|-------------|--------|----------|--------|
| 18 | ETFs (PSX-listed) | dps.psx.com.pk/etf/{SYM} | 🔴 HIGH | 1.1 |
| 19 | REITs (metadata enrichment) | SECP + DPS | 🟡 MED | 1.1 note |
| 20 | T-Bill Auction Results | sbp.org.pk/ecodata/ | 🔴 HIGH | 1.2 |
| 21 | PIB Auction Results | gsp.sbp.org.pk | 🔴 HIGH | 1.3 |
| 22 | Govt Ijara Sukuk Auctions | gsp.sbp.org.pk | 🟡 MED | 1.3 |
| 23 | PKRV Yield Curve | gsp.sbp.org.pk | 🔴 HIGH | 1.4 |
| 24 | KONIA Rates | gsp.sbp.org.pk | 🟡 MED | 1.4 |
| 25 | SBP FX Interbank | sbp.org.pk/ecodata/rates/WAR/ | 🔴 HIGH | 1.5 |
| 26 | SBP FX Open Market | sbp.org.pk/ecodata/rates/ECAP/ | 🔴 HIGH | 1.5 |
| 27 | Forex Kerb Rates | forex.pk | 🟡 MED | 1.5 |
| 28 | VPS Pension Funds | mufap.com.pk | 🟡 MED | 1.8 |
| 29 | Secondary Bond Trading | gsp.sbp.org.pk | 🟢 LOW | v3.1 |
| 30 | Commodity Prices | forex.pk / APIs | 🟢 LOW | v3.1 |
| 31 | IPO Calendar | dps.psx.com.pk/listings | 🟡 MED | 1.7 |
| 32 | Dividend History | DPS company pages | 🟡 MED | 1.6 |
| 33 | Economic Indicators | sbp.org.pk/ecodata/ | 🟢 LOW | v3.1 |

## A.3 Complete Endpoint Catalog

### PSX Data Portal (dps.psx.com.pk) — FREE, No Auth

| Endpoint | Format | Freq | Status |
|----------|--------|------|--------|
| `/timeseries/eod/{SYM}` | JSON | EOD | ✅ Already scraped |
| `/timeseries/int/{SYM}` | JSON | Live | ✅ Already scraped |
| `/company/{SYM}` | HTML+JSON | EOD | ✅ Already scraped |
| `/market-watch` | JSON | Live | ✅ Already scraped |
| `/indices` | JSON | Live | ✅ Already scraped |
| `/sector-summary` | JSON | Live | ✅ Already scraped |
| `/etf/{SYM}` | HTML+JSON | EOD | 🆕 Build in 1.1 |
| `/debt/{SYM}` | HTML+JSON | EOD | ✅ Already scraped |
| `/listings` | HTML | Weekly | 🆕 Build in 1.7 |
| `/announcements/secp` | HTML | Daily | ✅ Partial |
| `/announcements/companies` | HTML | Daily | ✅ Partial |
| `/download/closing_rates/{DATE}.pdf` | PDF | EOD | ✅ Already scraped |
| `/download/mkt_summary/{DATE}.Z` | Compressed | EOD | ✅ Already scraped |
| `/download/text/listed_cmp.lst.Z` | Compressed | Weekly | ✅ Already scraped |

### SBP (sbp.org.pk) — FREE, No Auth

| Endpoint | Format | Freq | Status |
|----------|--------|------|--------|
| `/ecodata/t-bills.asp` | HTML | Bi-weekly | 🆕 Build in 1.2 |
| `/ecodata/auction-tbills.pdf` | PDF | Bi-weekly | 🆕 Build in 1.2 |
| `/ecodata/auction-results.asp` | HTML | Bi-weekly | 🆕 Build in 1.2 |
| `/ecodata/rates/WAR/WAR-Current.asp` | HTML | Daily | 🆕 Build in 1.5 |
| `/ecodata/rates/ECAP/ECAP-Current.asp` | HTML | Daily | 🆕 Build in 1.5 |
| `/ecodata/rates/KIBOR/` | HTML | Daily | ✅ Already scraped |
| gsp.sbp.org.pk auction results | HTML | Bi-weekly | 🆕 Build in 1.3 |
| gsp.sbp.org.pk PKRV rates | HTML | Daily | 🆕 Build in 1.4 |
| gsp.sbp.org.pk KONIA rates | HTML | Daily | 🆕 Build in 1.4 |
| gsp.sbp.org.pk secondary market | HTML | Daily | 🟢 v3.1 |

### MUFAP (mufap.com.pk) — FREE, No Auth

| Endpoint | Format | Freq | Status |
|----------|--------|------|--------|
| `/Industry/IndustryStatDaily?tab=3` | HTML/JSON | Daily | ✅ Already scraped |
| `/Industry/IndustryStatDaily?tab=1` | HTML/JSON | Daily | ✅ Already scraped |
| `/FundProfile/FundDirectory` | HTML | Static | 🟢 Enhancement |
| VPS section | HTML | Daily | 🆕 Build in 1.8 |

### Third-Party FREE APIs

| Source | What | Limits | Status |
|--------|------|--------|--------|
| forex.pk | Kerb/dealer FX rates | Scrape | 🆕 Build in 1.5 |
| Open Exchange Rates | 170+ currencies vs USD | 1000/mo free | 🟢 Optional |
| ExchangeRate-API | PKR cross-rates | 1500/mo free | 🟢 Optional |
| Yahoo Finance (.KA suffix) | PSX EOD backup | Rate limited | 🟢 Backup only |

### Paid APIs (Future Reference)

| Source | Coverage | Price | When |
|--------|----------|-------|------|
| EODHD | PSX EOD + fundamentals | $30/mo | If DPS blocks |
| Alpha Vantage | Global stocks, FX | Free 25/day | Global comparisons |
| Twelve Data | Real-time + historical | Free 800/day | Real-time FX |
| PSX Official Feed | Level 1/2 real-time | Contact PSX | Production SaaS |

## A.4 PSX-Listed ETFs (as of Feb 2026)
...
## A.5 PSX-Listed REITs

| Symbol | Name | Type |
|--------|------|------|
| DLREIT | Dolmen City REIT | Rental |
| ARREIT | Arif Habib REIT | Developmental |
| SREIT | Signature Residency REIT | Rental (new 2026) |

## A.6 Data Refresh Schedule

| Data Domain | Frequency | Time (PKT) | Method | Cron (UTC) |
|-------------|-----------|------------|--------|------------|
| Equities EOD | Daily Mon-Fri | 18:30 | Async fetcher | 30 13 * * 1-5 |
| Intraday | Market hours | Every 5 min | Async fetcher | */5 4-10 * * 1-5 |
| Market watch | Market hours | Every 1 min | Async fetcher | Live |
| Indices | Daily Mon-Fri | 18:30 | With EOD sync | 30 13 * * 1-5 |
| ETF NAV | Daily Mon-Fri | 19:00 | ETF scraper | 0 14 * * 1-5 |
| Mutual Fund NAV | Daily Mon-Fri | 20:00 | MUFAP sync | 0 15 * * 1-5 |
| T-Bill auctions | Bi-weekly (Wed) | 19:00 | SBP scraper | 0 14 * * 3 |
| PIB auctions | Monthly | 19:00 | SBP scraper | 0 14 15 * * |
| PKRV curve | Daily Mon-Fri | 18:00 | SBP scraper | 0 13 * * 1-5 |
| KIBOR | Daily Mon-Fri | 12:00 | SBP scraper | 0 7 * * 1-5 |
| KONIA | Daily Mon-Fri | 18:00 | SBP scraper | 0 13 * * 1-5 |
| SBP FX rates | Daily Mon-Fri | 17:00 | SBP scraper | 0 12 * * 1-5 |
| Kerb FX | 3x daily | 10:00,14:00,17:00 | forex.pk | 0 5,9,12 * * 1-5 |
| Bonds/Sukuk | Daily Mon-Fri | 18:30 | With EOD sync | 30 13 * * 1-5 |
| Company profiles | Weekly (Fri) | 20:00 | Deep scrape | 0 15 * * 5 |
| Dividends | Daily Mon-Fri | 19:00 | Announcements | 0 14 * * 1-5 |
| IPO calendar | Weekly | 20:00 | DPS scraper | 0 15 * * 5 |
| DB maintenance | Monthly 1st | 03:00 | Vacuum+analyze | 0 22 1 * * |

## A.7 Existing PSX MCP Server Reference

A basic PSX MCP server exists (by ahad-raza24 on GitHub) that hits 3 DPS endpoints:
- `/market-watch` → market data
- `/timeseries/int/{SYM}` → intraday  
- `/timeseries/eod/{SYM}` → EOD

Your MCP server covers 25+ tools across 34 data domains — 10x more comprehensive.
