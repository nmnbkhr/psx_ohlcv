...
def _render_book_tab():
    ...
    if all_prices:
        ...
        fig_heat.update_layout(**PLOT_LAYOUT, height=400, title_text="Bid (green) / Ask (red) Volume",
...
def _render_simulation_tab():
    ...
    if sim.trade_log:
        with st.expander("Trade Log (first 100)"):
            st.dataframe(pd.DataFrame(sim.trade_log[:100]), use_container_width=True, hide_index=True)
...
def _render_history_tab():
    ...
    if hist_sub == "Book Snapshots":
        ...
        st.dataframe(run_df[show_cols] if show_cols else run_df, use_container_width=True, hide_index=True)
...
def render_page():
    ...
    tab_book, tab_sim, tab_rl, tab_hist, tab_research = st.tabs([
        "Book Visualization", "Market Simulation", "RL Agent", "History & Reporting", "Research"
    ])
