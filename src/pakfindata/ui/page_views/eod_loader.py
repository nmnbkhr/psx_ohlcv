...
def _eod_data_loader_page_impl():
    ...
    try:
        from pakfindata.api.client import get_client, is_api_available, APIError
    ...
    with tab_download:
        ...
        if api_available:
            ...
            if st.button("Download & Load", key="eod_dl_run", type="primary", disabled=not expected_dates):
                from pathlib import Path
                from pakfindata.config import DATA_ROOT
                from pakfindata.db import ingest_market_summary_csv, check_eod_date_exists
                from pakfindata.sources.market_summary import fetch_day_with_tracking, init_market_summary_tracking
        ...
        if not not_loaded:
            ...
            if st.button("Load Selected Dates", key="eod_load_selected", type="primary", disabled=not selected_dates):
                if api_available:
                    ...
                    # Direct DB access
                    from pakfindata.config import DATA_ROOT
                    from pakfindata.db import ingest_market_summary_csv
    ...
    with tab_tasks:
        ...
        if not api_available:
            ...
            st.code("uvicorn pakfindata.api.main:app --port 8000")
        else:
            try:
                client = get_client()
